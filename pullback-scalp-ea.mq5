...
int      ichH1[MAX_SYMS];      // Trend filter
int      ichM15[MAX_SYMS];     // Intermediate alignment + exit large profit
int      ichM5[MAX_SYMS];      // Exit small profit
int      ichM1[MAX_SYMS];      // Entry trigger + exit losing
int      adxH1[MAX_SYMS];      // H1 ADX for trend confirmation
int      atrM15[MAX_SYMS];     // M15 ATR for overextension + spread + profit measurement
...
//==============================================================
// M1 Pullback Detection: lost alignment within last N bars
//==============================================================
...
//==============================================================
// Overextension Guard: Price distance from M15 Kijun in ATR
//==============================================================
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(activeState[s] != 0)
      {
         ...
         if(!shouldExit)
         {
            ...
            {
               // Small profit — moderate stop: M5 Kijun
            ...
            {
               // Large profit — loose stop: M15 Kijun
      ...
      // Step 4: M1 must currently be fully aligned in same direction
